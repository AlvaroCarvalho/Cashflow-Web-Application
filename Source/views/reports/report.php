...
<div class = 'reports'>
    ...
    <form class = 'reports__filter'>
        ...
        <label class = 'input-container'>
            Turno
            <input type = "number" id = "filter__shift"/>
        </label>
        <label class = 'input-container'>
            Pagamento
            <select id="filter__payment">
        ...
        <label class = 'input-container'>
            Valor
            <input type = "number" id = "filter__amount" step="0.01"/>
        </label>
        <label class = 'input-container'>
...
?>
    <script>
        ...
        const getReport = async url => {
            ...
            const filterShift = document.getElementById("filter__shift").value;
            const filterPayment = document.getElementById("filter__payment").value;
            const filterAmount = document.getElementById("filter__amount").value;
        ...
        const renderBleedings = (bleedings, comments) => {
            const { mainContentContainer } = DOM();

            bleedings.forEach((el, i) => {
                let bleeding = `
                    <div class='bleeding'>
                        <h2>Sangria: Turno ${el.shift}</h2>`;

                for (key in el) {
                    if (bleedingFields(key)) {
                        bleeding += `<div class = 'input-container'>
                            ${bleedingFields(key)}
                            <input type='number' readonly='true' value='${el[key]}' />
                            <input type='text' readonly='true' value='${comments[i][key] ? comments[i][key] : ""}' />
                            <input type='checkbox' class='check' /> Ok
                        </div> `;
                    }
                }

                const others = JSON.parse(el.other);
                const comment = JSON.parse(comments[i].other);

                if (others.length) {
                    others.forEach((el, i) => {
                        bleeding += `<div class = 'input-container'>
                            Outros
                            <input type='number' readonly='true' value='${el}' />
                            <input type='text' readonly='true' value='${comment[i] ? comment[i] : ""}' />
                            <input type='checkbox' class='check' /> Ok
                        </div> `;
                    });
                }
                ...
                mainContentContainer.insertAdjacentHTML('beforeend', bleeding);
            });
        };

        const addCheckboxesListener = () => {
            const checkboxes = document.querySelectorAll('.check');
        
            checkboxes.forEach(el => {
                el.addEventListener('change', () => {
                    const value = el.parentNode.querySelector('input[type=number]');
                    const comment = el.parentNode.querySelector('input[type=text]');

                    if (el.checked) {
                        value.classList.add('positive');
                        comment.classList.add('positive');
                    } else {
                        value.classList.remove('positive');
                        comment.classList.remove('positive');
                    }
                });
            });
        };
        ...
        getReportButton.addEventListener("click", async e => {
            ...
            if (report.success) {
                ...
                addClosingListener();
                renderBleedings(report.data.bleedings, report.data.bleeding_comment);
